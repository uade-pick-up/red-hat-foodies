			...
			<ul id="courseplaycontent{{ $chapter->id }}" class="display-none">
				@foreach($chapter->courseclass as $class)
					...
					@if($class->type == 'video' && $class->iframe_url == NULL)
						@if($class->status == '1')
						<li  
						data-thumb-source="{{ url('images/course/'.$chapter->courses->preview_image) }}" data-video-source="{{ $url }}"

							data-start-at-time="{{date('H:i:s',strtotime($endtime))}}"
						
						    data-poster-source="{{ url('images/course/'.$chapter->courses->preview_image) }}"  data-subtitle-soruce="[
